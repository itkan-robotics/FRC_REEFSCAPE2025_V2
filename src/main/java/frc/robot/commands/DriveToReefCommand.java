...
public class DriveToReefCommand extends Command {
  ...
  public void execute() {
     ...
     * the one on either blue/red alliance c. The angle of the robot is within +-5 degrees of the
    ...
    if (LimelightHelpers.getTV(llName)
      ...
      /**
       * Calculate the velocity of the robot First, the horizontal movement is calculated using the
       * TX (horizonal angle) value of the camera (since we assume the robot is aligned ~95%)
       * Second, the vertical movement is calculated using the TA (Area) value of the camera (bigger
       * area, closer to target); this term has a setpoint of the area we see when we are right up
       * against the reef (~16%) Finally, we rotate this velocity by the angle of the reef we are
       * driving to since these calculations are always relative to the robot/AprilTag - Abdullah
       * Khaled's explanation (hope it makes sense!)
       */
      simpleLinearVelocity =
          new Translation2d(
    ...
    /**
     * Calculate the scalar of the drive based on a. The actual max speed of the robot b. The
     * slowdown multiplier based on the target elevator height
     */
    ...
    LoggedTunableNumber staticTolerance =
        new LoggedTunableNumber("simpleReefAlignment/minimumSpeedTolerance", 0.03);
